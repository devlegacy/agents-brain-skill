...
**Total nodes:** 0 plannings | 0 sessions | 0 concepts | 0 ADRs
...
> Valid operations: `bootstrap`, `ingest`, `update`, `plan`, `rename`, `merge`, `split`, `lint`, `deprecate`.
...
Wiki at: <WIKI_DIR>/
Command: /<COMMAND_NAME> (ingest | query | lint | plan <slug>)
...
<WIKI_DIR>/sessions/          ← session nodes (one per conversation)
<WIKI_DIR>/plannings/         ← planning nodes (living documents, span multiple sessions)
